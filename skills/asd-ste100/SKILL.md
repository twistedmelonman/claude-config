...
Vendored from [danyuchn/asd-ste100-skill](https://github.com/danyuchn/asd-ste100-skill) (MIT) at commit
`e4d64d1` (2026-08-30), skill version 0.4.0. Copied rather than cloned or submoduled so that
`~/.claude` stays a symlink-only runtime directory with no nested git repos.

This is a fork: local edits here do not flow upstream, and upstream changes do not arrive
automatically. **Occasionally check the source for updates to this skill** — compare the upstream
`version:` field in the frontmatter against `0.4.0` above:

```bash
gh api repos/danyuchn/asd-ste100-skill/commits --jq '.[0] | "\(.sha[0:7]) \(.commit.author.date) \(.commit.message | split("\n")[0])"'
```

Related: the always-on STE preference in `CLAUDE.md` ("Execution Preferences") sets when to write
in STE without being asked. This skill is the on-demand rewriter for text you point it at. Keep the
